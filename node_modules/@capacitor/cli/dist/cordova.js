...
async function autoGenerateConfig(config, cordovaPlugins, platform) {
    ...
    const cordovaConfigXMLFile = path_1.join(xmlDir, 'config.xml');
    ...
    cordovaPlugins.map(p => {
        ...
        if (currentPlatform) {
            ...
            if (configFiles) {
                const configXMLEntries = configFiles.filter(function (item) { return item.$.target === target; });
...
async function logiOSPlist(configElement, config, plugin) {
    ...
    if (!dict.key.includes(configElement.$.parent)) {
        ...
        common_1.logInfo(`plugin ${plugin.id} requires to add \n  ${xml} to your Info.plist to work`);
    }
}
