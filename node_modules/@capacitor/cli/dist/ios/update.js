...
function generatePodFile(config, plugins) {
    const capacitorPlugins = plugins.filter(p => plugin_1.getPluginType(p, platform) !== 2 /* Cordova */);
    const pods = capacitorPlugins
        .map((p) => `pod '${p.ios.name}', :path => '../../node_modules/${p.id}'`);
    const cordovaPlugins = plugins.filter(p => plugin_1.getPluginType(p, platform) === 2 /* Cordova */);
