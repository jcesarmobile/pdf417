...
async function updateAndroid(config) {
    ...
    while (needsPluginUpdate) {
        cordovaPlugins = plugins
            .filter(p => plugin_1.getPluginType(p, platform) === 2 /* Cordova */);
...
`;
    ...
    cordovaPlugins.map(p => {
        ...
        prefsArray = prefsArray.concat(plugin_1.getPlatformElement(p, platform, 'preference'));
    });
            ...
            if (resourceFiles) {
                resourceFiles.map((resourceFile) => {
                    if (resourceFile.$.src.split('.').pop() === 'aar') {
                        fs_1.copySync(plugin_1.getFilePath(config, p, resourceFile.$.src), path_1.join(pluginsPath, 'libs', resourceFile.$['target'].split('/').pop()));
                    }
                    else {
                        fs_1.copySync(plugin_1.getFilePath(config, p, resourceFile.$.src), path_1.join(pluginsPath, resourceFile.$['target']));
                    }
                });
            }
...
function removePluginsNativeFiles(config) {
    const pluginsRoot = path_1.resolve(config.app.rootDir, 'node_modules', '@capacitor/cli', 'assets', 'capacitor-android-plugins');
    const pluginsPath = path_1.join(pluginsRoot, 'src', 'main');
    fs_1.removeSync(path_1.join(pluginsRoot, 'gradle-files'));
